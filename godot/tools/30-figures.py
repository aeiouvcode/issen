...
def draw_player(c, J, pose):
    ...
    c.wash(body, dens=0.2, edge=0.45, layer='fill', rag=1.8, texture=0.45, streak_dir=1, grad=0.15)
    c.wash([tuple(shR), tuple(lerp(shR, hipR, 0.9)), tuple(lerp(J['hip'], hipR, 0.3)), tuple(lerp(J['sh'], shR, 0.4))], dens=0.35, edge=0.2, rag=2.2, texture=0.55, streak_dir=1)
    ...
    if not back:
        ...
        cx0 = -n * 3 * ay
        c.stroke([tuple(J['neck'] + n * 5 + cx0), tuple(lerp(J['neck'], J['hip'], 0.45) - n * 5 + cx0)], w=4.2, dry=0.5)
        c.stroke([tuple(J['neck'] - n * 4 + cx0), tuple(lerp(J['neck'], J['hip'], 0.3) + n * 3 + cx0)], w=3.0, dry=0.7)
    else:
...
def draw_ronin(c, J, pose):
    ...
    c.wash(body, dens=0.22, edge=0.4, rag=2.4, texture=0.5, streak_dir=1, grad=0.14)
